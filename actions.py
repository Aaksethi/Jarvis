...
# Where JARVIS creates new build projects. On Windows the Desktop is often
# redirected into OneDrive, so read the authoritative path from the registry.
def _resolve_desktop() -> Path:
    if IS_WINDOWS:
        try:
            ...
            log.warning(f"Could not read Desktop path from registry: {e}")
    return Path.home() / "Desktop"


DESKTOP_PATH = _resolve_desktop()

# Windows: launch processes in their own visible console window.
...
def _find_windows_chrome() -> str | None:
    """Locate chrome.exe in the usual Windows install paths."""
    candidates = [
...
def _spawn_windows_console(command: str) -> bool:
    """Open a new Windows console window running `command`, kept open after.

    Uses cmd.exe /k so the window stays so the user can watch progress.
    ...
    try:
        ...
        log.error(f"Failed to spawn Windows console: {e}")
        return False
...
async def open_terminal(command: str = "") -> dict:
    ...
    Windows: opens a new console window. macOS: Terminal.app via AppleScript.
    """
    if IS_WINDOWS:
        launched = _spawn_windows_console(command or "echo JARVIS terminal ready")
        return {
            "success": launched,
            "confirmation": "Terminal is open, sir." if launched else "I had trouble opening a terminal, sir.",
        }
...
async def open_browser(url: str, browser: str = "chrome") -> dict:
    ...
    if IS_WINDOWS:
        ...
        try:
            if chrome and browser.lower() != "firefox":
                subprocess.Popen([chrome, url])
                app_name = "Chrome"
            else:
                # Fall back to the OS default browser (handles Firefox / no Chrome).
                os.startfile(url)
                app_name = "Firefox" if browser.lower() == "firefox" else "your browser"
            return {"success": True, "confirmation": f"Pulled that up in {app_name}, sir."}
        except Exception as e:
            log.error(f"open_browser (Windows) failed: {e}")
            return {"success": False, "confirmation": "I ran into a problem opening the browser, sir."}
...
async def open_claude_in_project(project_dir: str, prompt: str) -> dict:
    ...
    if IS_WINDOWS:
        ...
        # cd /d handles drive changes; claude reads CLAUDE.md on startup.
        launched = _spawn_windows_console(f'cd /d "{project_dir}" && claude{skip}')
        return {
            "success": launched,
            "confirmation": "Claude Code is running in a terminal, sir. You can watch the progress."
            if launched else "Had trouble spawning Claude Code, sir.",
        }
