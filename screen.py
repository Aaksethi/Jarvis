...
def _windows_screenshot_png() -> bytes | None:
    """Capture the primary monitor on Windows and return PNG bytes.

    Uses the `mss` library (fast, pure-Python, no native deps).
    """
    try:
        import mss  # imported lazily so non-Windows installs don't need it
        import mss.tools
        with mss.mss() as sct:
        ...
        log.warning(f"Windows screenshot failed: {e}")
        return None


def _windows_active_windows() -> list[dict]:
    """Enumerate visible top-level windows on Windows via the Win32 API."""
    try:
        import ctypes
        ...
        log.warning(f"Windows window enumeration failed: {e}")
        return []
...
async def get_active_windows() -> list[dict]:
    ...
    """
    if IS_WINDOWS:
        return _windows_active_windows()
...
async def get_running_apps() -> list[str]:
    ...
    if IS_WINDOWS:
        # Window titles already cover this on Windows; avoid extra deps.
        return []
...
async def take_screenshot(display_only: bool = True) -> str | None:
    ...
    """
    if IS_WINDOWS:
        png = await asyncio.to_thread(_windows_screenshot_png)
        if png:
            log.info(f"Screenshot captured: {len(png)} bytes")
